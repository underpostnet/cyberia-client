...
#   DEV_PORT        HTTP port for the local static server  (default: 8082)
#   WS_URL          WebSocket URL baked into the WASM      (default: ws://localhost:8081/ws)
#                   Use wss:// for production/TLS origins.
#   API_BASE        REST API base URL baked into the WASM  (default: http://localhost:4005)
...
API_BASE="${API_BASE:-http://localhost:4005}"
...
# ── 4. Build ───────────────────────────────────────────────────────────────────
section "Building Cyberia Client (DEBUG) — WS_URL=$WS_URL  API_BASE=$API_BASE"
cd "$PROJECT_ROOT"

# Wipe cached objects when config.h is newer (fresh checkout or manual edit).
...
make -j"$MAKE_JOBS" -f Web.mk all BUILD_MODE=DEBUG WS_URL="$WS_URL" API_BASE="$API_BASE"
